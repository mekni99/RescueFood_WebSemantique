    ...
    <nav class="bg-white dark:bg-gray-900 fixed w-full z-20 top-0 start-0 border-b border-gray-200 dark:border-gray-600">
        ...
        <div class="items-center justify-between hidden w-full md:flex md:w-auto md:order-1" id="navbar-sticky">
          <ul class="flex flex-col p-4 md:p-0 mt-4 font-medium border border-gray-100 rounded-lg bg-gray-50 md:space-x-8 rtl:space-x-reverse md:flex-row md:mt-0 md:border-0 md:bg-white dark:bg-gray-800 md:dark:bg-gray-900 dark:border-gray-700">
            <li>
              <a href="#/restaurant/dons" class="block py-2 px-3 text-white bg-blue-700 rounded md:bg-transparent md:text-blue-700 md:p-0 md:dark:text-blue-500" aria-current="page">Home</a>
            </li>
            <li>
              <a href="/restaurant/dons" class="block py-2 px-3 text-gray-900 rounded hover:bg-gray-100 md:hover:bg-transparent md:hover:text-blue-700 md:p-0 md:dark:hover:text-blue-500 dark:text-white dark:hover:bg-gray-700 dark:hover:text-white md:dark:hover:bg-transparent dark:border-gray-700">about</a>
            </li>
            <li>
                <a href="/restaurant/dons#" onclick="scrollToSection('dons-show-section')" class="block py-2 px-3 text-gray-900 rounded hover:bg-gray-100 md:hover:bg-transparent md:hover:text-blue-700 md:p-0 md:dark:hover:text-blue-500 dark:text-white dark:hover:bg-gray-700 dark:hover:text-white md:dark:hover:bg-transparent dark:border-gray-700">
                    Services
                </a>
        ...
        <div class="bg-white rounded-lg shadow-xl pb-8">
            ...
            <div class="flex flex-col items-center -mt-20">
                ...
                <div class="flex items-center space-x-2 mt-2">
                    <p class="text-2xl">                <p class="text-gray-700">{{ old('firstname', auth()->user()->firstname) }}{{ old('lastname', auth()->user()->lastname) }}</p>
                </p>
                ...
                <p class="text-gray-700">{{ old('firstname', auth()->user()->firstname) }}{{ old('lastname', auth()->user()->lastname) }}</p>
                <p class="text-sm text-gray-500">{{ old('city', auth()->user()->city) }}"{{ old('address', auth()->user()->address) }}</p>
            </div>
        ...
        <div class="my-4 flex flex-col 2xl:flex-row space-y-4 2xl:space-y-0 2xl:space-x-4">
                ...
                <div class="flex-1 bg-white rounded-lg shadow-xl p-8">
                    ...
                    <ul class="mt-2 text-gray-700">
                        <li class="flex border-y py-2">
                            <span class="font-bold w-24">Restaurant Name :</span>
                            <span class="text-gray-700" > {{ old('username', auth()->user()->username) }}</span>
                        </li>
                        <li class="flex border-b py-2">
                            <span class="font-bold w-24">Email:</span>
                            <span class="text-gray-700">{{ old('email', auth()->user()->email) }}"</span>
                        </li>
                        <li class="flex border-b py-2">
                            <span class="font-bold w-24">FirstName:</span>
                            <span class="text-gray-700">{{ old('firstname', auth()->user()->firstname) }}</span>
                        </li>
                        <li class="flex border-b py-2">
                            <span class="font-bold w-24">lastname:</span>
                            <span class="text-gray-700">{{ old('lastname', auth()->user()->lastname) }}</span>
                        </li>
                        <li class="flex border-b py-2">
                            <span class="font-bold w-24">Location:</span>
                            <span class="text-gray-700"> {{ old('address', auth()->user()->address) }}</span>
                        </li>
                        <li class="flex border-b py-2">
                            <span class="font-bold w-24">City:</span>
                            <span class="text-gray-700">{{ old('city', auth()->user()->city) }}</span>
                        </li>
                        <li class="flex border-b py-2">
                            <span class="font-bold w-24">Languages:</span>
                            <span class="text-gray-700">{{ old('about', auth()->user()->about) }}</span>
                        </li>
            ...
            <div class="flex flex-col w-full 2xl:w-2/3">
                ...
                <div class="flex-1 bg-white rounded-lg shadow-xl mt-4 p-8">
                    <h4 class="text-xl text-gray-900 font-bold">Edit your profile</h4>
                    
                    <form role="form" method="POST" action="{{ route('profile.update') }}" enctype="multipart/form-data" class="mt-8">
                        @csrf
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                            <div>
                                <label for="username" class="block font-semibold mb-1">Username</label>
                                <input class="form-input w-full border border-gray-300 rounded-md p-2 focus:outline-none focus:ring-2 focus:ring-blue-500" type="text" name="username" value="{{ old('username', auth()->user()->username) }}">
                            </div>
                            <div>
                                <label for="email" class="block font-semibold mb-1">Email address</label>
                                <input class="form-input w-full border border-gray-300 rounded-md p-2 focus:outline-none focus:ring-2 focus:ring-blue-500" type="email" name="email" value="{{ old('email', auth()->user()->email) }}">
                            </div>
                            <div>
                                <label for="firstname" class="block font-semibold mb-1">First name</label>
                                <input class="form-input w-full border border-gray-300 rounded-md p-2 focus:outline-none focus:ring-2 focus:ring-blue-500" type="text" name="firstname" value="{{ old('firstname', auth()->user()->firstname) }}">
                            </div>
                            <div>
                                <label for="lastname" class="block font-semibold mb-1">Last name</label>
                                <input class="form-input w-full border border-gray-300 rounded-md p-2 focus:outline-none focus:ring-2 focus:ring-blue-500" type="text" name="lastname" value="{{ old('lastname', auth()->user()->lastname) }}">
                            </div>
                            <div>
                                <label for="address" class="block font-semibold mb-1">Address</label>
                                <input class="form-input w-full border border-gray-300 rounded-md p-2 focus:outline-none focus:ring-2 focus:ring-blue-500" type="text" name="address" value="{{ old('address', auth()->user()->address) }}">
                            </div>
                            <div>
                                <label for="city" class="block font-semibold mb-1">City</label>
                                <input class="form-input w-full border border-gray-300 rounded-md p-2 focus:outline-none focus:ring-2 focus:ring-blue-500" type="text" name="city" value="{{ old('city', auth()->user()->city) }}">
                            </div>
                            <div>
                                <label for="country" class="block font-semibold mb-1">Country</label>
                                <input class="form-input w-full border border-gray-300 rounded-md p-2 focus:outline-none focus:ring-2 focus:ring-blue-500" type="text" name="country" value="{{ old('country', auth()->user()->country) }}">
                            </div>
                            <div>
                                <label for="postal" class="block font-semibold mb-1">Postal code</label>
                                <input class="form-input w-full border border-gray-300 rounded-md p-2 focus:outline-none focus:ring-2 focus:ring-blue-500" type="text" name="postal" value="{{ old('postal', auth()->user()->postal) }}">
                            </div>
                            <div class="col-span-2">
                                <label for="about" class="block font-semibold mb-1">About me</label>
                                <textarea class="form-textarea w-full border border-gray-300 rounded-md p-2 focus:outline-none focus:ring-2 focus:ring-blue-500" name="about">{{ old('about', auth()->user()->about) }}</textarea>
                            </div>
                        </div>
                        <div class="text-center mt-6">
                            <button type="submit" class="bg-blue-600 text-white font-semibold rounded-md py-2 px-4 hover:bg-blue-700 transition duration-200">Save</button>
                        </div>
                    </form>
                    <div class="flex-1 bg-white rounded-lg shadow-xl p-8">
                        <h4 class="text-xl text-gray-900 font-bold">About</h4>
                        <p class="mt-2 text-gray-700">{{ old('about', auth()->user()->about) }}</p>
                    </div>
